...
window.onload = function() {
	...
	// FF editor -----------------------------------------------
	FF.displayCurrentTrackRaw();
	FF.analyseCurrentTrackRaw();
	let newTrackCoords = [];
	counter = 0;
	latAccu = 0;
	longAccu = 0;
	tsAccu	= 0;
	tracks.currentItem().rawPositions.forEach(function(position) {
		FF.filterAccuracyLess50(position,newTrackCoords);
	});
	console.log(newTrackCoords)
	tracks.list.unshift(new Track({label:"filterAccuracyLess50",coords:newTrackCoords,color:'#880088',rotatorIcon:'fas fa-project-diagram'}));
